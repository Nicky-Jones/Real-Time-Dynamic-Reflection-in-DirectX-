...
vertexOutputPacket main(vertexInputPacket inputVertex) {
	...
	outputVertex.tangentW = 0;//mul(float4(inputVertex.tangent, 1.0f), (float3x3)worldITMatrix); //(float3x3)worldITMatrix
	// Pass through material properties
	outputVertex.matDiffuse = mul(inputVertex.matDiffuse, 2.0f);
	...
	//outputVertex.posH = mul(float4(inputVertex.pos, 1.0f), worldMatrix);
